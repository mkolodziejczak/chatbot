<?php
  ...
  function make_like_pattern($sentence, $field)
  {
    ...
    foreach ($words as $word)
    {
	  ...
	  $mid_lp = str_replace(' %', " $last_word", $mid_lp);
      $sql_like_pattern .= "  $mid_lp OR\n";
	  $save= str_replace(" $last_word", " $last_word %", $mid_lp);
      $mid_lp = preg_replace("/$first_word/", "% $first_word", $mid_lp, 1);
	foreach ($words as $word)
	{
	  $mid_lp = preg_replace("/% $word/", '%', $mid_lp, 1);
	  if($save != '%')
      $sql_like_pattern .= "  $mid_lp OR\n";
	}
	  $save = preg_replace("/$first_word/", "% $first_word", $save, 1);
	foreach ($words as $word)
	{
	  $save = preg_replace("/% $word/", '%', $save, 1);
	  if($save != '% %')	
      $sql_like_pattern .= "  $save OR\n";
	}
